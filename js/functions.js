...
- Creats filters for all countries and cities belonging to each country.

NO RETURN VALUE

//create_country
ARGUMENTS
country: an object from array COUNTRIES that contains the following keys:
- id: (a number value) the country's id.
- name: (string) the country's name.
No argument validation is performed.

SIDE_EFFECTS
- Creates a new DOM element for a country and appends it accordingly
- Sets the text content and CSS classes for the DOM element.
- Filters every city that has a "country" ID that matches the country's ID.
- Calls the create_city function for each object in the filtered cities array

NO RETURN VALUE

//create_city
ARGUMENTS
city: an object fron the array CITIES that contains the following keys:
...
SIDE_EFFECTS
-creates a new DOM element for a city and appends it inside the corresponding country
DOM element.
- Gives the city DOM elemnt a class to be selected by default.
- Sets the text content for the city DOM element.
- Stores the city ID in the dataset attribute of the DOM elemnt.
...
/*
 ...
 //create_filters
 ARGUMENTS
 type: level, subject, data
 - The type of filter to create.
 array: LEVELS, SUBJECTS, LANGUAGES
 - The array of objects that will be converted into new DOM elements.
 No argument validation is performed.

 SIDE_EFFECTS
 - Create new DOM elements for every object in the array.
 - Appends the DOM elemts on the website based on the type of filter they are.
 - Assigns CSS classes to the new DOM element
 - Stores the IDs of the filter type inside the dataset attribute of the new DOM elements. 
...
function update_programmes() {
  ...
  */
  ...
  if (array.lenght !== 0) {
    ...
    document.querySelector("#programmes > p").style.displayy = "inline";
  }
...
/*
 ...
 //read_filters
  ...
  RETURN VALUE
  - Returns an array: programmes which contains the programmes that has properties that match the filters and search field
*/
